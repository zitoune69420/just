...
/* Fondu du bandeau : un vrai masque alpha (et non un aplat de couleur par
 * dessus), l'image se dissout donc dans le fond de page ET dans les halos
 * décoratifs. Cantonné au dernier quart, avec des arrêts intermédiaires
 * (#000b / #0004) qui adoucissent l'entrée du dégradé : une rampe linéaire
 * courte laisse voir une bande au point de départ.
 * `-webkit-mask-image` est requis avant Chrome 120 / Safari 15.4, seuls
 * navigateurs modernes à ne pas connaître la version non préfixée. */
const BACKDROP_FADE =
...
/* Copies floutées superposées : chacune est révélée un peu plus bas que la
 * précédente, ce qui fait monter le flou par paliers fondus l'un dans l'autre
 * (flou progressif) au lieu d'une simple disparition nette. Le flou monte en
 * même temps que la transparence et va jusqu'à 40px : en bas il ne reste
 * aucun détail net, donc aucun bord perceptible.
 * `scale-*` pousse hors cadre les bords adoucis par le flou (sinon ils
 * laissent des liserés translucides sur les côtés) ; il suit le rayon de flou,
 * un écart trop grand sur la couche la moins floue se verrait en dédoublement. */
...
export function MediaDetailView({ details }: { details: MediaDetails }) {
  ...
  return (
    <article className="enter pb-16">
      {/* Bandeau backdrop fondu dans le fond de page. */}
      <div
      ...
      >
        {backdropUrl && (
          <>
            ...
            />
            {/* Même URL que la couche nette : aucune requête supplémentaire.
                `fill` place déjà les copies en absolute inset-0. */}
        ...
        {/* Voile sombre (masqué avec le reste du bandeau) : garde le badge et
            le haut de l'affiche lisibles par dessus l'image. Resserré sur la
            même zone que le flou, sinon il forme à lui seul un long dégradé
            visible en haut du bandeau. */}
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <header className="relative z-10 -mt-36 flex flex-col gap-6 sm:-mt-44 sm:flex-row sm:items-end sm:gap-8">
          <div className="relative aspect-2/3 w-40 shrink-0 overflow-hidden rounded-3xl shadow-2xl ring-1 ring-border/60 sm:w-56">
            {details.poster ? (
              <Image
          ...
          <div className="max-w-2xl space-y-4 pb-1">
            ...
            {/* Le tracking suit la taille : plus le texte grandit, plus les
                lettres doivent se resserrer pour garder la même densité. */}
